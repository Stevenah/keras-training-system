...
def write_model_summary(file_name, metrics, experiment_name, folds, table_size):

    with open(file_name, 'w') as file:

        file.write(pad_string("", table_size, "-", "-"))
        file.write(pad_stringf(f" model summary for {experiment_name} using {folds} folds", table_size, " ", "|"))
        file.write(pad_string("", table_size, "-", "|"))
