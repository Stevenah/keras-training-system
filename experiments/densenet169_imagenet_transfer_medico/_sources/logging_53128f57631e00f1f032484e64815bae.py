...
def log_misclassifications( file_name, misclassifications, class_name, classes):
    ...
    division_point  = max(map(len, classes.values()))

    for misclassification_file_name, misclassification_info in misclassifications.items():
        table_content.append(f'{ misclassification_file_name } misclassified as { misclassification_info["prediction"] }')
        table_content.append(TABLE_ROW_DIVIDER)
        for label, prediction in misclassification_info['predictions'].items():
            table_content.append(f'{ label.ljust(division_point) } { prediction }')
