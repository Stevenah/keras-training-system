...
def run( config, config_path ):
    ...
    experiment_name = f'{experiment.path}_run_{experiment.current_run._id}'
    ...
    for split_index in range(len(split_dirs)):
        ...
        model_file = config['model'].get('load', None)

        if model_file is not None:
            model_dir = config['model']['base_dir']
            model = load_model( os.path.join( model_dir, f'split_{ split_index }_{ model_file }' ) )
        else:
            # import model
