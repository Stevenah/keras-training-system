...
def get_kvasir_labels(order='label_index'):
...
def prepare_dataset(split_dirs, temp_directory, split_index, split_total):
    training_dir   = os.path.join(temp_directory, 'training_data')
    validation_dir = os.path.join(temp_directory, 'validation_data')
...
def split_data(folds, data_dir, temp_dir):
    ...
    for index in range(folds):
        split_dirs.append(os.path.join(temp_dir, 'splits', f'split_{index}'))
