...
#     total = max (labels_dict.values())

#     class_weight = dict()

#     for key in labels_dict.keys():
#         print(f'{key}: {labels_dict[key]}')
#         score = 2 - float(labels_dict[key]) / total
...
def train( model, config, experiment, training_directory=None,
    ...
    if config['hyper_parameters']['class_weights']:
        for class_index, class_name in enumerate(class_list):
            class_dispersion[class_index] = len(os.listdir(os.path.join(training_directory, class_name)))
