...
def run():
    ...
    for split_index in range(len(split_dirs)):
        ...
        last_weights = train(model, config, experiment, training_directory, validation_directory)
        ...
        if config['fine_tuning']['mode'] == 'enabled': 
            ...
            config = json.load(open(config['fine_tuning']['link']))

            # train using new config
            train(model, config, experiment, training_directory, validation_directory) 

        model = ModelHelper(model, number_of_classes, image_width, image_height, image_channels)

        # evaluate train model and get metrics
        split_results = evaluate(model, config, validation_directory, experiment) 
